...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // 첫 줄, 첫 글자 초기화
        char tempChar = currLine.charAtr(0);
        ...
        // 첫 번째 색상이 하얀색 보드와 다르다면,
        if(whiteBoardColor != tempChar) {
            whiteLineDiff++;
            whiteDiff[0][0] = whiteLineDiff;
        }
        
        // 첫 번째 색상이 검은색 보드와 다르다면,
        if(blackBoardColor != tempChar) {
            blackLineDiff++;
            blackDiff[0][0] = blackLineDiff;
        }
        ...
        for(int i = 1; i < N; i++) {
            ...
            for(int j = 0; j < M; j++) {
                ...
                // 첫 번째 열 이라면, 이전 줄의 마지막 열을 가져온다.
                if(j == 0) {
                    whiteDiff[i - 1][M - 1];
                    blackDiff[i - 1][M - 1];
                } else {
                    // 첫 번째 열이 아니라면, 앞의 열을 가져온다.
                ...
                if(i >= 7 && j >= 7) {
                    ...
                    if(eraseRow >= 0) {
                        upperWhiteDiff = whiteDiff[i][j] - whiteDiff[eraseRow][M - 1];
                        upperBlackDiff = blackDiff[i][j] - blackDiff[eraseRow][M - 1];
                    }
                    ...
                    int rightWhiteDiff = whiteDiff[i][eraseCol] - whiteDiff[eraseRow][eraseCol];
                    int rightBlackDiff = blackDiff[i][eraseCol] - blackDiff[eraseRow][eraseCol];
